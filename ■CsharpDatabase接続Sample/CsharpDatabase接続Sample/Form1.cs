...
namespace CsharpDatabase接続Sample
{
    public partial class Form1 : Form
    {
        ...
        */
        ...
        public void 複数データの取得()
        {
            ...
            using (var command = sqlConnection.CreateCommand()) //ここで[sqlConnection] → [SqlCommand] に
            {
                ...
                try
                {
                    ...
                    //☆SQL文の設定
                    command.CommandText = @" SELECT BusinessEntityID, FirstName, MiddleName, LastName FROM Person.Person WHERE BusinessEntityID BETWEEN 1 AND 20 ORDER BY BusinessEntityID ";

                    #region ------------他の方法：SqlCommandを定数で表す(SQL文の設定からExecuteReader()メソッドの呼び出しまで)-------------

                    //☆SQL文の設定
                    const string cmdText = @" SELECT BusinessEntityID, FirstName, MiddleName, LastName FROM Person.Person WHERE BusinessEntityID BETWEEN 1 AND 20 ORDER BY BusinessEntityID ";
                    ...
                    {
                        //ビジネスid/firstName/middleName/lastName 取得
                    }
        ...
        private void IPアドレス手入力()
        {
            uiパネル.Enabled = uiIPアドレス手入力.Checked;
        }
    }
}
